#!/usr/bin/env python3
"""
chunk.py — Split text files into ~N-word chunks for multi-pass editing.

MODES
-----
split      Split a text file into chunks on paragraph/sentence boundaries.
...
-----
Split to files:
    python3 chunk.py split input.md --output-dir ./chunks/

Split and print with delimiters (useful for piping):
    python3 chunk.py split input.md --print

Custom chunk size:
    python3 chunk.py split input.md --words 1000 --output-dir ./chunks/

Reassemble:
    python3 chunk.py reassemble ./chunks/ --output output.md
    python3 chunk.py reassemble ./chunks/ --print
...
the current chunk at that paragraph boundary and starts a new one. This
means chunks may run slightly over or under the target — a complete
paragraph is always kept together.

If a single paragraph exceeds the target word count (uncommon in prose),
it is kept in one chunk rather than split mid-paragraph.
"""
...
def split_into_paragraphs(text: str) -> list[str]:
    ...
    Returns a list of paragraph strings (without the separating blank lines).
    Blank-line separators are preserved as empty strings to allow exact
    reconstruction of whitespace.
    """
    # Normalize line endings
    text = text.replace("\r\n", "\n").replace("\r", "\n")

    # Split on one or more blank lines; keep separators as items
    parts = re.split(r"(\n\s*\n)", text)
...
def build_chunks(text: str, target_words: int = 1500) -> list[dict]:
    """
...
def cmd_reassemble(args: argparse.Namespace) -> None:
    ...
    # Collect all chunk_NNN.* files and sort numerically
    chunk_files = sorted(
    ...
    # Join with a blank line between chunks to restore paragraph separation
    assembled = "\n\n".join(parts) + "\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split text files into ~N-word chunks for multi-pass editing, or reassemble them.",
    ...
    split_parser.add_argument("input", help="Path to the input text/markdown file.")
    split_parser.add_argument(
        "--words",
        type=int,
        default=1500,
        metavar="N",
        help="Target words per chunk (default: 1500). Chunks may run slightly over.",
    )
